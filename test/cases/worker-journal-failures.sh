...
sudo composer-cli --json compose start bash | tee "$COMPOSE_START"
...
sudo composer-cli log "$COMPOSE_ID" | sudo tee "$LOG_OUTPUT"
...
for i in "${!EXPECTED_OUTPUT[@]}"; do
    grep "${EXPECTED_OUTPUT[$i]}" "$JOURNAL_OUTPUT"
    grep "${EXPECTED_OUTPUT[$i]}" "$LOG_OUTPUT"
done
# the worker also logs the JobError which includes the entire error in one line as part of the error details
grep "failure details.*Failed to enable unit: Unit blergh.service does not exist" "$JOURNAL_OUTPUT"
